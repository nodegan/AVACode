# Running AVA Code in the Background

On a Linux host, AVA Code can run as a background service for your user. It starts when the machine
boots and keeps running after you log out.

## Manage the Service

Install it with the latest AVA Code release:
...
Updating restarts AVA Code briefly. Let active agent work and terminal commands finish first.
If a remote update is already in progress, wait for it to finish before retrying a local update.

The systemd unit runs a small stable launcher. Exact AVA Code versions are installed separately, so
...
want AVA Code to start in the background.
