...
This guide is for people who want to use more than one Codex account in AVA Code. For Claude, see
[Claude](./providers-claude.md). For first-time setup, see [Install AVA Code](./install.md).
...
In AVA Code Settings, name it something obvious:
...
In AVA Code Settings, add another Codex provider:
...
AVA Code shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, AVA Code treats it
