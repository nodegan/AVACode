...
1. Creates three temporary T3 base directories and starts a local server for each on an available
   port.
2. Creates AVA Code, React, and Linux Git repositories with recognizable favicons, feature branches,
   and a deterministic AVA Code review diff.
...
The Pending rows use the production offline outbox and point at the real AVA Code and React fixture
